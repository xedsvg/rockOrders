        ...
        toastIdRef.current = toast.show({
          ...
          render: () => (<Alert maxWidth="100%" alignSelf="center" flexDirection="row" status="success" variant="left-accent" mr={5}>
            <VStack space={1} flexShrink={1} w="100%" alignItems="center">
              ...
              {dismissCallback ? <Button variant="subtle" colorScheme="success" onPress={ async () => {dismissCallback(); toast.close(toastIdRef.current); toastIdRef.current = null;}} > On my way!</Button>: null}
            </VStack>
          </Alert>)
        });
      }
